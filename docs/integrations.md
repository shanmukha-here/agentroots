...
MLflow adapter uses read-only REST lookup. Trackio adapter injects version-specific fetch.
Neither writes or launches runs. Flowcept/AiiDA are interfaces only. agentic-experiments/signac
importers normalize compact H→E→F and signac job summaries without copying execution semantics.
